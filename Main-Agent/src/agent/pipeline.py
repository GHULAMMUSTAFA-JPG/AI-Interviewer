...
async def process_candidate_message(
    ...
    try:
        ...
        # ===== STAGE 3: CALL LLM =====
        # Cancellation is the concurrency mechanism (handled in main.py).
        # If a newer message arrives, this task gets cancelled at the next await.
        ...
        try:
            if needs_summary:
                combined_prompt = _build_combined_prompt(
                    ...
                    phase=context.phase,
                    phase_turn_count=context.phase_turn_count,
            ...
            logger_struct.info("stage3_cancelled", interview_id=context.interview_id)
        ...
        # ===== STALE CHECK (safety net between Stage 4 and Stage 5) =====
        # Primary mechanism is task cancellation (main.py). This is a backup:
        # if cancellation raced with Stage 3 completing, discard the response
        # rather than sending a reply to an outdated message.
        try:
        ...
        # phase_turn_count was loaded from DB in Stage 1 context; increment for this turn
        phase_turn_count = context.phase_turn_count + 1
